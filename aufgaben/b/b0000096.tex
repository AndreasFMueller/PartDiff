...
and $u(-1) = u(1) = 0$ on the boundary of $\Omega$.

Determine an approximation function $\tilde u(x)$ for $u(x)$ by the
method of Galerkin. 

Use the ansatz $\tilde u(x) = a_1 \cdot (1-x^2) + a_2 \cdot (1-x^4).$
...
Since
\[
\int_{-1}^1 1 - x^2 \ dx = 4/3, \ \ \ \int_{-1}^1 (1-x^2) \cdot x^2 dx = 4/15,
\]
as well as
\[
\int_{-1}^1 1 - x^4 \ dx = 8/5, \ \ \ \int_{-1}^1 (1 - x^4) \cdot x^2 \ dx = 8/21,
\]
...
\tag{\textbf{2P}}
\end{equation}
Therefore $(a_1, a_2) = (0, -1/12).$
and
$\tilde u(x) =  1/12 \cdot (x^4 - 1)$ (\textbf{1P}).
...
with
\begin{equation}
r = \int_0^1 4 \cdot (x - x^2) \cdot x^2 \ dx = 1/5.
\tag{\textbf{1P}}
\end{equation}
