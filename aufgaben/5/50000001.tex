...
Wir schreiben $\mathscr{L}$ für die Laplace-Transformation von $s$.
Die transformierte Differentialgleichung ist
\[
s\mathscr{L}u(s, x)-u(0,x)=\kappa \partial_x^2\mathscr{L}u(s,x).
\]
Setzt man die Anfangsbedingung $u(0,x)=0$ ein, wird daraus
\[
s\mathscr{L}u(s, x)=\kappa \partial_x^2\mathscr{L}u(s,x),
\]
also eine mit $s$ parametrisierte Familie von gewöhnlichen
Differentialgleichung der Funktion $x\mapsto \mathscr{L}u(s,x)$.
Etwas umgestellt ist sie äquivalent zu
\[
\partial_x^2\mathscr{L}u(s,x)
=
\frac{s}{\kappa}\mathscr{L}u(s, x),
\]
mit den Lösungen
\[
\mathscr{L}u(s,x)=\begin{cases}
