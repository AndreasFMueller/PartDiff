...
die Laplace-Trans\-for\-ma\-tion kennt:
\[
\mathscr{L}J_0(s)=\frac1{\sqrt{s^2+1}}
\]
...
soweit dies in ge\-schlos\-se\-ner Form möglich ist.
...
\[
x\frac{\partial\mathscr{L}u(x,s)}{\partial x}
-
u(x,0)+s\mathscr{L}u(x,s)=0.
\]
...
s\mathscr{L}u(x,s)=0.
\]
Die Anfangsbedingung für $\mathscr{L}u(x,s)$ für $x=1$ ist die
Laplacetransformierte von $1-J_0(t)$, also
\[
\mathscr{L}u(1,s)=\frac1s-\frac{1}{\sqrt{s^2+1}}.
\]
Weil $\sqrt{s^2+1}>\sqrt{s^2}=s$ ist, kann man folgern, dass
die Laplacetransformierte $\mathscr{L}u(1,s)$ für alle $s$ negativ ist.
\item
Die Theorie der gewöhnlichen Differentialgleichungen garantiert, dass
die Funktionen $\mathscr{L}u(x,s)$ eindeutig festgelegt sind, damit folgt
auch, dass $u(x,t)$ eindeutig festgelegt ist.
\item
Schreiben wir $y_s(x)=\mathscr{L}u(x,s)$ erhalten wir die Differentialgleichung
...
\[
\mathscr{L}\bigl(f(t-\log x)\theta(t-\log x)\bigr)
=
e^{-s\log x}\mathscr{L}(f(t)),
\]
