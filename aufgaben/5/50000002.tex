...
die Fourier-Trans\-for\-mier\-te und erhalten $\hat u(k,0)=\hat f(k)$.
...
\begin{align*}
(\mathscr{L}\partial_x^2u)(x,s)
+
s^2(\mathscr{L}u)(x, s)-u(x,0)-s\partial_xu(x,0)
&=0\\
\partial_x^2(\mathscr{L}u)(x,s)+s^2(\mathscr{L}u)(x, s)-u(x,0)-s\partial_xu(x,0)=0
\end{align*}
Die Anfangsbedingung liefert $u(x,0)=f(x)$, also
\[
\partial_x^2(\mathscr{L}u)(x,s)+s^2(\mathscr{L}u)(x, s)-f(x)-s\partial_xu(x,0)=0
\]
Dies ist eine Differentialgleichung zweiter Ordnung für jedes $s$, schreiben
wir $(\mathscr{L}u)=y_s(x)$, und zur Abkürzung $g(x)=\partial_xu(x,0)$,
lautet sie
\begin{align*}
