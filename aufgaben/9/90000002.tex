...
\item Lösen Sie die Differentialgleichung nach $\frac{dy}{dx}$ auf.
...
\]
Teilt man durch $\dot x^2$, erhält man die Gleichung
\begin{align*}
...
+(1-x)^2&=0\\
\end{align*}
\item Die Auflösung nach $\frac{dy}{dx}$ erfolgt mit der Lösungsformel
...
\end{align*}
\item
Die beiden Charakteristiken sind die Lösungen der Differentialgleichung
für die verschiedenen Vorzeichen $\pm$. $\frac{dy}{dx}$ ist die
Steigung der Charakteristiken, für $x=0$ ergibt sich
\[
\frac{dy}{dx}
=
-\frac{1\pm\sqrt{5}}{2}
...
können die Winkel zur $x$-Achse und damit der Schnittwinkel
