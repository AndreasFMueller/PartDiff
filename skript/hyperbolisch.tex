...
Der Graph einer Funktion $u(t,x)$ ist eine Fl"ache im dreidimensionalen
Raum mit Normale
$$
\begin{pmatrix}
\partial_t u\\\partial_x u\\-1
\end{pmatrix}
$$
F"ur eine L"osung der PDGL (\ref{wellelinks}) gilt
$$
...
\qquad
\text{mit}\quad
\vec v =\begin{pmatrix}1\\-a\\0\end{pmatrix}
$$
insbesondere steht die Normale auf dem Vektor $\vec v$ senkrecht.
Dies bedeutet aber auch, dass der Vektor $\vec v$ eine Tangente an
den Graphen ist.
Der Graph ensteht also als Fl"ache dadurch, dass die Anfangsbedingung
$u_0(x)$ mit Hilfe des Vektors $\vec v$ verschoben wird.
Der Punkt $(0,x_0,u_0(x_0))$ wird nach der Zeit $t$ in den Punkt
$(t,x_0-at,u_0(x_0))$ verschoben worden sein.

Umgekehrt kann man daraus auch den Funktionswert im Punkt $(t,x)$
berechnen. Der Punkt $(t,x)$ wird vom Punkt $x_0=x+at$ aus erreicht,
also ist die $z$-Komponente auf dem Graphen
$$u(t,x)=u_0(x+at).$$
Als Funktion von $x$ ist
$u(t,x)$ als eine um $at$ nach links verschobene Kopie von $u_0$.
...
\begin{equation}
\partial_tu(0,x)=v_0(x)\label{welleanfangdt}.
\end{equation}
...
\begin{align*}
u(0,x)&=u_0(x)\\
\partial_tu(0,x)&=\frac12\bigl(au_0'(x+at)+v_0(x)-au_0'(x)+v_0(x)\bigr) =v_0(x)
\end{align*}
Somit l"asst sich die Wellengleichung in der Ebene einfach durch Finden einer
Stammfunktion f"ur eine Funktion entlang konstruieren. Das Anfangs des Abschnittes
gestellte Problem entspricht dem Fall $v_0(x)=0$.

\section{Das Cauchy-Problem}
...
%%% CHANGE
\marginpar{\tiny In diesem Abschnitt waren im urspr"unglich
publizierten Skript die Rollen von $x'$ und $t'$ vertauscht.}
